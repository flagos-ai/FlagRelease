...
if [ "${QUALIFIED_CORE}" = "True" ] && [ "${SEG4V4_DONE}" = "no" ] && [ -n "${SEG_CTR}" ] && docker inspect --type=container "${SEG_CTR}" &>/dev/null; then
...
if [ -f "${SHARED_CTX}" ]; then
    ...
    if [ "${QUALIFIED_CORE}" != "True" ]; then
        echo ""
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] qualified_core=${QUALIFIED_CORE}（service_ok/accuracy_ok 未同时满足），跳过段4.5（V4 减算子）"
...
if [ "${QUALIFIED_CORE}" = "True" ] && [ -n "${SEG_CTR}" ] && docker inspect --type=container "${SEG_CTR}" &>/dev/null; then
...
if [ -f "${SHARED_CTX}" ]; then
    ...
    if [ "${QUALIFIED_CORE}" != "True" ]; then
        echo ""
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] qualified_core=${QUALIFIED_CORE}（service_ok/accuracy_ok 未同时满足），跳过段5（V5 算子扩展）"
