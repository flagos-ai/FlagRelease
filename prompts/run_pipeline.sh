...
- 步骤 11/12 不达标 → **三级递进**（精度/性能同此逻辑，不直接放弃）：
  ① 先写 issue 到 flagos-ai/vllm-plugin-FL 记录问题；
  ② plugin 模式关算子调优：operator_search.py run --plugin-mode --final-output-name v3_performance --state-path /flagos-workspace/results/operator_config_v3.json（走 env_inline VLLM_FL_FLAGOS_BLACKLIST，在已达标算子集基础上继续关拖累算子直到达标）；调优达标即置 accuracy_ok/performance_ok=true 继续；
  ③ 全关 flaggems 算子仍不达标 → 判定为框架问题，提交 plugin-error issue（标注全关仍不达标），保持 accuracy_ok/performance_ok=false 继续步骤 13。
- 步骤 13 触发条件：plugin_workflow.accuracy_ok=true AND performance_ok=true
- 所有 issue 提交到 flagos-ai/vllm-plugin-FL（非 FlagGems）
- 算子集以主流程已达标版本为起点；**达标则不重新调优，不达标则进入上述三级递进允许在 plugin 模式下继续调优**
...
# 多源兜底：V5="应开尽开"，只要任何阶段禁用过算子就应扩展。单读 optimization.disabled_ops
# 太脆弱（Claude 曾把禁用写到 workflow.v2_disabled_ops 等错误字段 → 误判无禁用 → 漏跑 V5）。
# 因此从多个来源判断，任一显示"有算子曾被禁用/当前启用集 < 初始全量集"即触发 V5。
