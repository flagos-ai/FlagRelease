...
目标：性能 ≥ V3，接近/超过 V1。硬约束：**至少保留 1 个算子**（保底 flaggems 生效）。
...
def run_accuracy_guard(gpqa_script: str, gpqa_config: str, output_path: str,
    ...
    """轻量精度护栏：移除算子后精度相对基线退化不超过 guard%。

    返回 (passed, score)。基线分数为 0 时跳过校验（passed=True）。
    """
    if nv_or_v1_score <= 0:
        return True, 0.0
...
def run_reduction(
    ...
    print(f"  目标: 性能 ≥ V3，接近/超过 V1 | 保底至少 1 个算子")
    print(f"{'=' * 60}\n")
    ...
    while len(current_ops) > 1 and round_num < round_limit:
        round_num += 1
...
def main():
    ...
    parser.add_argument("--accuracy-baseline", type=float, default=0.0,
                        help="精度护栏基线分数 (NV 或 V1，0=跳过精度校验)")
    ...
    if len(enabled_ops) <= 1:
        print(f"⚠ V3 仅 {len(enabled_ops)} 个算子，已满足保底约束，无需精简")
