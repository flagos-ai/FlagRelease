...
4. **上传成功一律以 V3 镜像为准**（达标只看 V3）。优先查找 `versions.v3.harbor_image` 非空或 `traces/13_plugin_release.json` 中 harbor_push status=success；**特殊场景**：V2=V3 双 tag 同镜像时（分支 B 无独立 V1 或仅 v1.3 成功），V3 镜像在步骤 8 发布且 `versions.v2.harbor_image` 同时带 `-v3` 后缀，此时若 `versions.v2.harbor_image` 包含 `-v3` tag 或 `traces/08_release.json` 显示双 tag 上传成功，也判定 uploaded=true；证据不足时 uploaded=null，明确失败时 uploaded=false。即使额外产出了 V4 镜像，uploaded 仍只反映 V3 是否上传成功。
